...
本書はロードマップ（`cela_roadmap_v24.md`）のR1・R2・R3の3フェーズ分をまとめて詳細化したものである。実装着手時に、ロードマップのどのフェーズが本書のどの節に対応するかが分かるよう、以下に対応表を示す。**実装は必ずこの表の左列（R番号）の順序で進め、対応する節だけを参照すること。** R4・R5は別紙（`cela_r4_design.md`、`cela_r5_design_v2.md`）を参照。
