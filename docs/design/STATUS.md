...
6. [x] BL-012: B.5.1既知誤判定の非退行テストを`test_f26_detection.py`に追加（D-011、2026-07-19完了。実行はユーザー指示待ち）
7. [ ] **R2ドライラン実施（ユーザー指示待ち）**: `tests/test_f26_detection.py`の実LLM呼び出し実行（指標D）、指標Cの複数試行比較。実施後`traceability.md`・`impl_Plan.md` R2.9に記録
8. [ ] BL-002: 評価メトリクスA・B・Cの実測比較（R2ドライラン完了後、P2-2）
9. [x] BL-003: Record&Replayスタブの実LLM応答による往復検証（T-5、2026-07-18完了）
10. [x] BL-009: `done`（許容方針をコードコメントで明示済み、継続監視のみ）
11. [ ] BL-011: 当面見送り（D-010、MVP後・P2）
