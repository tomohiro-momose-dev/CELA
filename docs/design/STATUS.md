...
| 項目 | 状態 |
|------|------|
| **アクティブ Phase** | Phase 4 / R4（ホワイトボード差分パッチ化）— R3a・R3bはDone、BL-041（facilitator/Arbiter再設計）は設計ドラフトのみで実装は保留し、検証コスト低減を優先してR4に着手 |
| **Phase 状態** | R3a/R3b: 実装完了（オフラインスモークテスト50件Pass、実ドライランで検証、BL-039/040は`done`、BL-038は原因未確定で`open`）。R4: 実装計画策定中（Plan mode） |
| **次マイルストーン** | R4実装計画の策定・承認 → 実装（`whiteboard_drafts`差分パッチ化、BL-034/040のファイルベース版管理との統合可否を含む） → A/Bテスト（トークン消費・矛盾の早期発見率・ロールバック正確性） → 指標C実測 → BL-005/BL-041（facilitator/Arbiter再設計）着手 |
...
| Phase 3 (R3a) | **未着手（設計・実装待ち、2026-07-21〜）** | D-036により新設。F-3.8（自律的DB/ファイル読み取りツール）・F-3.9（構造化ファクトストア、`{topic, value, reason, citations}`、confirmed/provisional区別）を実装し、BL-035（フェーズ横断`verified_facts`参照不能）・BL-036（財務・需要数値ドリフト）・BL-037（`reason_why`の薄さ）を解消する。完了後、BL-035〜037と同一構成での再ドライランと指標C実測を行う（`phase_gates.md` P3a-1〜P3a-4）。R3b（`write_agreement_tool`、旧来のR3スコープ）はR3a完了後に着手。 |
