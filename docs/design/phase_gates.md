...
| P2-2 | 評価メトリクスA（却下案の回避率）・B（制約の維持率）・C（収束性とコストのトレードオフ、最低5試行）の実測比較（[BL-002](issue_backlog.md)、[D-002](decision_log.md)） | `traceability.md` T-* | ☐（指標D実測はT-7・T-8で完了。A・B・Cは未実施） |
