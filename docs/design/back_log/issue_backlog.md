...
| BL-145 | 低 | `cela_main.py`（`write_issue`のDEFER、`task_planner_node`の`plan_revision_reason`再計画経路、`reflection_node`のBL-144滞留追跡、`state["phases"]`の構造データ） | `log/2026-08-02/0832`のOrchestrator関連バグ調査（Bug A/B/C、BL-146〜148予定）と合わせて、ユーザーから「majorとなったissueや後続タスクへの申し送りは、Detector/Reflectorの正当性監査後、タスクプランナー経由で明示的にタスク化した方がissue消化がスムーズになるのでは」との設計提案。調査の結果、現状`write_issue`のDEFERは対象task_idの`plan_drafts`（文書ホワイトボード）へ申し送りテキストを追記するのみで、issueを正式なtask_idとして`state["phases"]`（コード側のゲートが実際に信頼する構造データ本体）へ組み込む経路は存在しないことを確認。また`state["phases"]`自体は`task_planner_node`が`plan_revision_reason`非空時（task_plan_reviewerの差し戻し／Essence Dialogue収束の2経路のみ）に`existing_phases`全体をLLMで再生成する方式であり、`plan_drafts`のような差分パッチAPIを持たない。ユーザーから「タスクの修正が今後ぽろぽろ出てくるはずなので、1タスク追加・削除・修正の軽量APIが欲しい」との追加要望あり——これは本提案の実現方式そのものにも直結する前提条件として記録する。**状態**: `open`（設計相談のみ、実装未着手）。 | P3 |
