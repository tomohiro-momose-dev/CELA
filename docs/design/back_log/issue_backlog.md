...
| BL-141 | 中 | `cela_main.py`（`expert_node`のchat_history追記ロジック） | ユーザーから「detectorとエキスパートの差戻しループは見せずにuser->エキスパートの履歴だけに整理できませんか？差戻ループが続くと最初に言ったユーザー発言が見れなくなる恐れがあり、次のユーザーが前のユーザーが何を言ったかわからなくなってしまいます」との指摘。調査したところ、Detectorがmajor判定で差し戻すと`route_after_expert_detector`がUserの発言を挟まず`expert_node`へ直接ループバックし（最大3回）、`expert_node`は差し戻しのたびに新規の`assistant`メッセージを**無条件で**`chat_history`へ追記していたことが判明。`chat_history_window`（既定4）は全ノードで一律に「直近N件」を切り出す方式のため、同一タスクの差し戻し往復だけで4件のウィンドウが埋まり、Userの直近の指示や別タスクの履歴がウィンドウ外へ押し出される実害があった。**実装完了（`done`）**: `expert_node`末尾で、`chat_history`の直前のエントリが既に`assistant`（＝同一ターン内のDetector差し戻しによる再提出）であれば新規追記せず上書きするよう変更。Userから見える`chat_history`はUser⇄Expertの最終提出のみに整理される（差し戻し往復自体はagreements DB経由で別途監査可能なため、chat_history側で消えても実害はない）。issueについては、`generate_user_utterance`に既に「Expertの回答がその懸念を解消していれば`write_issue(RESOLVE)`でクローズしてください」という指示が存在することを確認済み（別途強制力の強化は今回のスコープ外）。新規テスト`tests/test_bl141_expert_retry_chat_history_collapse.py`（3件: 通常追記・単発差し戻し上書き・3連続差し戻しでも1エントリに収束することの確認）追加。`python -m py_compile`合格、フルオフラインスイート563件Pass。 | P1 |
