...
| BL-131 | 中 | `cela_main.py`（`write_agreement`のtask_id検証、task_planner計画との整合） | `log/2026-07-29/1322`レビューで発見。Facilitatorが提案した「制約見直し要求書」を`task_1_1_review`という**task_plannerの正式なフェーズ・タスク計画に存在しないtask_id**でExpertが実行し、244行・6バージョンの成果物を作成してしまった。`write_agreement`/ホワイトボード作成時にtask_idが実在の計画（`state["phases"]`）と照合されず、任意の文字列で成果物が作れてしまう。`current_task_id`が空文字列になる等、正規のタスク進行状態から外れるリスクがある。BL-126の「本質対話→合意→task_planner再構成→正式タスク化」フローが整備されれば防げる可能性が高いが、それとは別に`write_agreement`側でtask_idを検証する独立した安全策も検討の余地がある。**状態**: `open`（基本設計完了、実装は未着手。[BL126_basic_design.md](BL-126/BL126_basic_design.md)にBL-126/130と一体で保存。実装順序上は最小・独立のため最初に着手する位置づけ）。 | P2 |
