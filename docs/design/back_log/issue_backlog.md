...
| 項目 | 内容 |
|------|------|
| 状態 | `open`（一部実装済み。下記「同じ検証・計算を繰り返さない」グループは`done`、`read_entity`専用警告グループは未着手） |
| 優先度 | P2 |
| テスト | `tests/test_bl256_verification_throttle_warning_consolidation.py`（新規8件）。実装済みグループ分。`read_entity`専用警告グループの実装時に追加予定 |
