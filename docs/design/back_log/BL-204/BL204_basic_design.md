...
> **改訂履歴**
> - 初版: 2026-08-10
> - 第2版: Clineによる独立レビューの指摘4件を実コードと突き合わせて検証し反映
>   （§2.1.1で`assumption`を削除、`confirmed_by`へ改名、§2.4にalias・属性名の
>   スコープ判断を追記、§4へ2項目追加）。あわせて§6の要確認事項4点をユーザー判断で確定。
>   レビュー指摘のうち2件は、指摘された「正当化を書き足す」対応ではなく
>   **原因そのものを除去する**対応を選んだ（詳細は§2.1.1・§2.4）。
...
#### エイリアスの扱い（Clineレビュー指摘・軽4への対応）

初版は`entities.aliases`をスキーマに置きながら、その登録経路と検証方法を定義して
いなかった。レビューの指摘どおり、**モデルが自由にaliasを追加できると同一性ガードは
そこから抜ける**（「長野大学」をaliasとして登録すれば以後素通りする）。

v1では**モデルからのalias入力を一切受け付けない**。`aliases`列は残すが、設定するのは
2.3のゴール文初期登録のみとする。表記ゆれ（ゴール文の`組合立諏訪中央病院`に対して
Expertが`諏訪中央病院`と書く等）は、ガードが`did_you_mean`で正典名を提示し、Expertが
正典名で再発行することで解決する。1往復増えるが、抜け道はゼロになる。

#### 属性名の表記ゆれ（Clineレビュー指摘・軽3への対応）

初版は`read_entity`に`attr_name`指定を想定していたが、`attr_name`が完全自由である以上
読み取り側の表記ゆれ（`coordinates` vs `coordinate`）が問題になる、という指摘を受けた。

緩和策（近似一致）を足すのではなく、**問題自体を消す**方針を採る。v1の`read_entity`は
`attr_name`引数を持たず、**その事物の全属性を返す**（1事物あたり数〜十数件で、
コストは無視できる）。読み取り側で属性名を推測する場面自体が発生しない。

書き込み側は`attr_name`を必要とするため、**新しい`attr_name`を作成したときだけ、
その事物の既存属性名一覧をレスポンスへ添える**。`coordinate`を新規作成した直後に
「既存: address, coordinates, elevation_m」と返れば、重複作成に気づける。
新しいあいまい一致サブシステムを導入せずに済む。
...
## 6. 決定事項（初版の要確認事項に対するユーザー判断）

1. **ゴール文からの初期登録は`task_planner`ノード内で行う**。`build_graph`のトポロジーは
   変更しない。BL-201/BL-203で「resume経路だけ挙動が違う」事故を続けて経験しているため、
   グラフ構造を触らない選択は再発リスクの面でも妥当。既存の再計画スキップガード
   （`phases`確定済みなら再実行しない）と同型の冪等ガードを入れ、resumeで二重登録しない。

2. **新規ツールは4本**（`register_entity`を独立させる）。`write_entity_attribute`の
   フラグへ畳む案より意図が明示的で、登録時のみ`citations`必須といったバリデーションも
   分けやすい。Expertは17→20本になる。

3. **v1から全事物型を対象**とする（`place`に限定しない）。この帰結として、
   **Expert/Detectorのプロンプト誘導はドメイン非依存の一般的表現で書く**
   （地理・GIS等、特定ゴールに寄せた記述を書かない。BL-196でユーザーが明示した規律）。
   `verify_entity_geo`のみ、住所と座標を持つ事物にだけ適用される旨を明記する。

4. **`revise_goal`によるゴール改定時の再検証は、BL-168の先例に揃える**
   （`cela_main.py:2399`付近。改定時に該当`verified_facts`の`reason`へ「整合性未確認」
   警告を付記する処理）。同じ扱いを`entities`/`entity_attributes`にも適用し、
   新しい扱いを発明しない。
