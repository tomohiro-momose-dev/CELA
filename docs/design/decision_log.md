...
| 影響 | `cela_main.py`（`_StreamRepetitionGuard`クラス＋4定数の新設、`_query_AI_live`の2分岐への配線、計6箇所）、`tests/test_bl297_stream_repetition_guard.py`（新規9件）。AGENTS.md §17.1確認済み（cela_main.py全体をgit stashで巻き戻し、新規9件全てが失敗することを確認後、diffが完全一致することを確認して復元）。既存の`test_bl231_loop_guard.py`・`test_bl287_tool_repeat_nudge.py`（計8件）で非退行を確認。この機構はストリーミングチャンクの実際の受信タイミングに依存するため、実LLM呼び出しでの効果確認（実際の崩壊を早期に打ち切れるか、正当な長い生成を誤って打ち切らないか）は次回ドライラン待ち。 |
