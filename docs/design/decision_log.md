...
| 影響 | `cela_main.py`（`LineageState`定義、`decision_extractor_node`のUPDATE分岐、デバッグprint2箇所）、`tests/test_r4_smoke.py`（回帰テスト2件追加）。オフラインスモークテスト66件全通過を確認。実LLM再ドライランでの最終確認は未実施。 |
