...
| **決定理由** | 1917ログ（run_id=1787890406-1e73a89d）の調査で、Deliverable旧レコード検索が`topic`文字列一致のみ・`task_id`不問で行われ（`cela_main.py:17957-17964`）、task_4_2の承認済みポインタがtask_4_1のホワイトボードスロットへ誤って向く識別子不整合を発見。AIは軽量案（該当箇所へ`task_id`一致条件を追加）を提示したが、同型の未検証topic一致パターンが独立に4箇所（`4047`/`4113`/`4247`/`_find_prior_superseded`）追加で存在し、AGENTS.md §15.1違反が5箇所に及ぶ構造的パターンであること、かつBL-206/210/211/212/329（§13.7）と同系譜の識別子不整合バグの6件目であることが判明した。ユーザーはこの重複の実証を踏まえ、「タスクプランナー・phase/task_idをlineageの様にDAG型にして機械的な紐づけを行う」方向性の採用を判断した。あわせて「CELAは当初DBを持たないstate onlyの簡易アーキテクチャで、各ノード連携の実証確認が目的だった。実証は十分済んだため、今後は堅牢な構造（グラフ等）へ各所を改める」というarchitecture全体の方向性を明言した。 |
