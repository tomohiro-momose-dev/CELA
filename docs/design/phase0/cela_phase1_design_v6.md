...
```sql
-- 1. 判断の系譜テーブル（コア、★v2でフルスキーマ準拠に修正、★v5でenum修正、★v6でinternal_thought_process追加）
CREATE TABLE IF NOT EXISTS agreements (
    ...
    internal_thought_process TEXT,     -- ★v6追加（Cline/hy3レビュー指摘①）: ツール実行直前の思考過程・スクラッチパッド。要件定義書4.2には存在していたが本設計書で欠落していた。R1でNULL許容カラムとして先行追加、書き込みはR5（F-3.7）で実装（decisionsテーブルの同名列と対で扱う）
...
-- 2. 共有ホワイトボード（成果物コア、★v6でtask_idを復活）
CREATE TABLE IF NOT EXISTS whiteboard_drafts (
    draft_id TEXT PRIMARY KEY,
    phase_id TEXT NOT NULL,            -- ★v2追加: 既存プロトタイプはタスク単位で成果物を持つため
    task_id TEXT NOT NULL,             -- ★v6追加（Cline/hy3レビュー指摘①）: v2〜v5で誤って欠落していた。要件定義書4.4に既に存在し、タスク単位の成果物管理（F-7）に必須のため復元
...
CREATE INDEX IF NOT EXISTS idx_whiteboard_drafts_phase_task ON whiteboard_drafts(phase_id, task_id);  -- ★v6追加: 要件定義書4.4と同一の複合インデックス

-- 3. 絶対目標（北極星、goal_idは常に'GLOBAL_GOAL'固定。★v6で要件定義書側との整合を明記）
...
### 3.1 v1のグラフ構造は撤回する（★v3で「維持」の意味を明確化、★v6で図を実構造に修正）
...
## 3.5 R2実装の技術詳細（★v6新設：Cline/hy3レビュー指摘③への回答）
...
## 3.6 R1実装の技術詳細（★v6新設：Cline/hy3レビュー指摘④への回答）

### 3.6.1 SQLite接続管理（F-13準拠）

```python
import sqlite3

def get_db_connection(db_path: str = "cela.db") -> sqlite3.Connection:
    conn = sqlite3.connect(db_path, timeout=60.0)  # F-13: タイムアウト60秒
...
**シングルライターキューの実装方針**: 既存プロトタイプはLangGraphの逐次実行（単一スレッド）であるため、R1時点では複数スレッドからの同時書き込みは発生しない。したがって、シングルライターキュー（バックグラウンドの書き込み専用スレッド経由での直列化）の本格実装は、R1では見送り、R5以降でF-11（非同期並行スレッド化・MCTS-Fork）に着手する段階まで不要とする。R1では単純な`sqlite3.connect`の使い回し（WALモード＋タイムアウトのみ）で十分と判断する。

### 3.6.2 `state["agreements"]`からSQLiteへの移行手順

既存コードで`state["agreements"].append(...)`が直接呼ばれている箇所（`decision_extractor_node`内、複数箇所）を、以下の手順で置き換える。

1. **ラッパー関数の導入**: `state["agreements"].append(agreement_dict)`をそのまま`db_write_agreement(agreement_dict, db_connection)`に置換する薄いラッパーを用意し、呼び出し側のコード変更を最小化する。
2. **読み出し側の置換**: `_build_agreements_context(state["agreements"])`のような、`state["agreements"]`をイテレートしている箇所は、`_build_agreements_context_from_db(db_connection)`に置換し、SQLiteへのSELECTクエリで同等のフィルタリング（`status != "Superseded"`等）を行う。
3. **`state`からの`agreements`キー撤去**: 上記1・2が完了した時点で、`LineageState` TypedDictから`agreements`フィールド自体を削除し、SQLite接続オブジェクト（またはdbパス文字列）を新たに保持させる。`decisions`についても同様の手順を踏む。
4. **回帰確認**: 評価メトリクスA（既存プロトタイプ vs SQLite版で`decisions`/`agreements`の内容が完全一致すること、§5参照）で移行の正しさを確認してから次のステップ（R2）に進む。

### 3.6.3 Hydrateコンテキストの受け渡しインターフェース

`assemble_hydrate_context`（§4のアセンブル擬似コード）は、呼び出し元（`call_expert`、`generate_user_utterance`等）に対して、以下のシグネチャで文字列を返すインターフェースとする。

```python
def assemble_hydrate_context(db_connection: sqlite3.Connection, phase_id: str | None = None) -> str:
    """
...
### 5.1 テスト・ドライラン手順（★v6新設：Cline/hy3レビュー指摘⑦⑧への回答）
