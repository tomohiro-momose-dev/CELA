...
> 3. **BL-034（Deliverableファイル保存のタイミング問題）・BL-040（ファイル名の`_Vn`バージョニング＋`old/`退避）との関係**: 本設計の`whiteboard_drafts`テーブルは`version`列を持ち、SQLite TEXT型で全文を保持できるため、BL-034/BL-040が対象としていた「ファイルシステム上でのバージョン管理」と役割が重複する。R4実装時に、Deliverableの正本を`whiteboard_drafts`テーブルに一本化し、ファイル書き出し（`save_deliverable_to_file`）を廃止するか、人間可読性のための副次的コピーとして残すかを決定する必要がある（実装計画側で扱う）。
