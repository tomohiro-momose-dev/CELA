...
| 2026-07-23 | 論点50を追記。ユーザーがBL-051（issue_blリスト＋フェーズ終了ゲート、論点48で起票）の軽量版として、Detectorの出力に気づき・懸念の自由記述欄を追加し後続ノードのプロンプトに提示するよう指示。本格版（構造化リスト・フェーズゲート）は設計未確定のまま`open`に据え置き、`observations`フィールド・`detector_observations_log`・`_build_detector_observations_block`のみ実装しBL-051を`partial`化。 |
