...
| BL-051 | 中 | `cela_main.py` (`call_detector`, `detector_node`, フェーズ終了判定ロジック) | Detectorの現行出力（`risk`/`constraint_issue`/`comment`の単一判定）では、思考過程で気づいた致命的でない懸念が構造化されずに失われる問題をユーザーが指摘。本格版（issue_bl的な構造化リスト＋フェーズ終了ゲート、BL-041と同格の構造変更）は`open`のまま据え置き、軽量版として`observations`自由記述欄の追加・`detector_observations_log`蓄積・User AI/Expertプロンプトへの毎ターン提示を実装（`partial`） | P2 |
...
- 実LLM再ドライランで、task間のオペレーター人数矛盾（3名シフト制 vs 2名常駐固定）のような非数値的懸念を、ドメイン妥当性レビューパスが実際に検出できるかを確認する（次回待ち）。
...
| 項目 | 内容 |
|------|------|
| 状態 | `partial`（軽量版のみ実装済み。フェーズ終了ゲート等の本格版は`open`のまま） |
| 優先度 | P2 |
...
| 2026-07-23 | ユーザーがBL-051（issue_blリスト＋フェーズ終了ゲート）の軽量版として、Detectorの出力に「思考過程で気づいたこと・懸念事項」を自由記述させる欄を追加し、後続ノードのプロンプトに提示するよう指示（フェーズ終了ゲート等の本格版は引き続き設計未確定のため見送り）。`call_detector`の両監査パスの返却JSONに`observations`を追加、`LineageState`に`detector_observations_log`を新設し`constraint_issue`の値に関わらず（none判定でも）蓄積、新規ヘルパー`_build_detector_observations_block`を`call_expert`・`generate_user_utterance`の両プロンプトに配線し直近3件を常時提示。BL-051を`open`から`partial`に更新。オフラインスモークテスト計71件Pass、`python -m py_compile`合格。 |
