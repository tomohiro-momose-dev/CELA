...
| BL-016 | 高 | `cela_main.py` (`detector_node`, ツールループ) | 本番ドライランでExpertが探索的タスク（車両配車の組合せ最適化）に入り、Detectorの完全性判定の硬直性から10回ツール呼び出しを使い切り非収束クラッシュ。実質的な結論はiter=9で出ていた | P0 |
| BL-017 | 中 | 新機能（未着手、設計要） | 「差し戻しループ沼」からの脱出機構（ファシリテーター／そもそも論への立ち返り）。BL-016と同根の問題への別解 | P2 |
...
- ツールループ側で残りiter数をモデルに意識させ、残り僅少時に強制的にテキスト最終回答を促す仕組みを検討する。
- Detectorの完了度判定に「明示的な申し送り事項付きの部分回答」を合格として扱う経路を追加するか検討する（数値矛盾チェックの厳格さは維持し、完了度チェックのみ緩和する）。
- 上記いずれかまたは両方の対応方針を`decision_log.md`にD-xxxとして記録し、AGENTS.md §5.2の設計ブループリントを提示した上で実装する。
- 実装後、同種の探索的タスク（組合せ最適化を要するシナリオ）で再ドライランし、非収束が解消することを確認する。
...
- 「差し戻しループ沼」または「局所改善の反復」をどう検知するか（同一ノードへの差し戻し回数、同一トピックへのDetector major判定の連続回数など）を設計する。
- 検知後の介在方法（新規ファシリテーターノードの追加か、既存orchestrator/detectorへの機能追加か）を設計する。
- AGENTS.md §5.2の設計ブループリントを提示しユーザー承認を得てから実装する。BL-016（直接対応）の効果を見てから着手するかどうかも含め、優先順位をユーザーと確認する。
