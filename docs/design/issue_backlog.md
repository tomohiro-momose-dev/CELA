...
| 項目 | 内容 |
|------|------|
| 状態 | `open`（未着手、設計要。ユーザーにより最優先対応と位置づけ） |
| 優先度 | P1 |
| 依存 | なし（BL-002の指標C計測はむしろ本Issue完了後に着手すべき） |
| 関連 | [BL-002](issue_backlog.md#bl-002-r1完了条件の実データabドライラン未実施)、[BL-018](issue_backlog.md#bl-018-task_planner由来のタスク間依存関係が状態に構造化されておらず横断的な影響判断ができない)（別軸、下記参照）、[D-018](decision_log.md#d-018-r4ホワイトボード編集方式はコーディングエージェント方式遅延取得機械的照合パッチを採用する)、`log/2026-07-19/2056/log_no_prompt.md` |
...
- task_plannerのプロンプト・出力スキーマを見直し、現状「1タスク＝1つの大きな複合成果物」となりがちな粒度を、独立に検証可能な単位（例：車両台数算出・初期費用内訳・年間ランニングコスト・感度分析をそれぞれ別タスクにする等）まで細分化する設計を検討する。
- 細分化のしすぎ（タスク数増大によるorchestrator/User AIのオーバーヘッド増加、タスク間依存関係の複雑化）とのバランスを検討する（BL-018のタスク依存構造化とも関連しうる）。
- 細分化後、同種のタスク（task_1.2相当）で差し戻しラウンド数・ツール呼び出し回数が実際に減ることをオフライン・実機の両方で確認する。
- 指標C（BL-002）の本格計測は、本Issueの対応が完了してから着手する方針とする。
- AGENTS.md §5.2の設計ブループリントを提示しユーザー承認を得てから実装する。
