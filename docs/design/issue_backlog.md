...
| BL-017 | 中 | `cela_main.py`（既存`facilitator_node`の拡張要） | 「差し戻しループ沼」からの脱出機構。ファシリテーター自体は既存（`facilitator_node`）だが、差し戻しループ検知をトリガーに持たない。BL-016と同根の問題への別解 | P2 |
