...
| `AI_plan.md` | **今回どう実装するか**（スライス・触るファイル・検証） |
| `Cursor Agent.md` | **何をしたか**（技術日記・教師データ。タスク状態は持たない） |
...
2. Agent に **全体開発計画** を起こさせる（Phase 分割・依存関係・リスク）。出力先は `AI_plan.md` の冒頭セクション。
...
3. 着手前に `AI_plan.md` にスライス表（触るファイル・検証コマンド）。
...
- 計画: `@AI_plan.md` の「Phase N」セクションを読んでから実装開始
...
- [ ] コード変更あり → `AI_plan` に計画 or 実績メモ
- [ ] タスク状態変化 → `BACKLOG` open/closed
- [ ] 技術判断あり → `Cursor Agent.md` 末尾（またはフック自動追記）
...
| 実装計画 | `AI_plan.md` + `.cursor/rules/ai-plan-implementation.mdc` |
| 技術日記 | `Cursor Agent.md` + `DevLogWriter`（`<!-- npu: ... -->`） |
...
A. いいえ。ヘッダ + 該当 FR leaf + BACKLOG + AI_plan で足りる場面が多い。

**Q. 脇道で調べた結論を本線に戻すには？**  
A. 結論を BACKLOG / AI_plan に 1 行で固定し、本線で Refresh。脇道チャット自体は本線の仕様根拠にしない。
