...
# 評価する対象（UserかExpertか）によって、チェック基準の厳しさを変える
    ...
    domain_prompt = (
        ...
        f"ことが必須です。\n"
        f"ただし、この「不明である」という判断自体を鵜呑みにする前に、それが本当に調べようがない"
        f"事項か、それとも法定基準・実在の相場・公的統計等、通常read_goal_reference/"
        f"read_reference_file/web_searchで確認できる事項かを区別してください。後者であるにも"
        f"かかわらず確認せずに「情報不足だから不明」で済ませている場合は、まず実際に確認してから"
        f"判定してください。確認してもなお不明・非公開の場合や、そもそも公的に確認しようのない"
        f"事項は、引き続きmajorの根拠にしないでください。\n\n"
