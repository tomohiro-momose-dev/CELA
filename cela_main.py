...
# ツール一覧の周知と、ゴール文・既存成果物にある定性的な言及（「商業施設が複数点在」
# 「小中学校」「冬季は冷え込む」等）の深堀りにも適用する。
...
_BL192_DIRECTIVE_QUALITY_BLOCK = (
    ...
    "書き込んでください（例：「◯◯の座標が必要な場合はgsi_geocodeで番地まで解決してから"
    "calc_road_routeで道路距離を求めること」）。\n"
    "④【定性的な言及を具体的な事物・数値へ深堀りする】ゴール文や既存の成果物には、種類・"
    "存在だけを述べて具体的な数・所在・数値を伴わない記述（例：「スーパー・商業施設は"
    "複数点在している」「小中学校、高校」「冬季は冷え込む」）が含まれることがあります。"
    "これをそのまま言い換えて成果物へ再掲するだけでは足りません。そのタスクの容量算定・"
    "SLA判定・受入基準の根拠として使う事物・数値であれば、実際に何件／何校あるか、"
    "どこにあるか、実際の利用実態はどうか、公式記録でどの地点の・どの程度の数値かを"
    "web_search等で調べ、register_entity/write_entity_attributeまたは"
    "write_agreementのconfirmed_variablesへ出典付きで登録するよう、指示文の中で"
    "具体的に名指ししてください。ただし、そのタスクのacceptance_criteria達成に不要な"
    "深堀りまで際限なく要求しないでください（①②③④はすべて、指示文を『抽象的な依頼』から"
    "『具体的な行動計画』へ変えるためのものであり、受入基準自体を拡張するものではありません）。\n"
)
...
def call_expert(expert_name: str, state: LineageState, config: Appconfig) -> str:
    ...
    system_prompt += (
        ...
        "ゴール文や既存の成果物には、種類・存在だけを述べて具体的な数・所在・数値を伴わない"
        "記述（例：「スーパー・商業施設は複数点在している」「小中学校、高校」「冬季は冷え込む」）"
        "が含まれることがあります。これをそのまま言い換えて成果物へ再掲するだけでは足りません。"
        "容量算定・SLA判定・受入基準の根拠として使う事物・数値であれば、実際に何件／何校あるか、"
        "どこにあるか、実際の利用実態はどうかをweb_searchで調べ、判明した事実を"
        "register_entity／write_entity_attribute（固有の名前を持つ対象、位置が要る場合は"
        "gsi_geocodeも併用）またはwrite_agreementのconfirmed_variables（単独の数値）へ"
        "出典付きで登録してください。例：\n"
        "- 「商業施設が複数点在」→ 実際にどの店舗が何件、どこにあるか\n"
        "- 「小中学校」→ 実際に何校あり、通学区はどう区分され、対象年齢層が本当に本サービスを"
        "通学の移動手段として使う可能性があるか（既に送迎バス・徒歩通学が主手段の場合、"
        "需要を過大評価してしまいます）\n"
        "- 「冬季は冷え込む」→ 気象庁等の公式記録で、どの観測地点で・実際に何度まで下がるか"
        "（市街地と山間・高原部では観測地点も気温も異なるため、代表点の記録を全域へ流用しない）\n"
