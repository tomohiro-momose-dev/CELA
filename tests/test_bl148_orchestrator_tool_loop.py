...
2. tools=に読み取り専用ツール（read_project_plan/read_deliverable_file/read_verified_fact/think）
   に加え、[BL-280]でwrite_agreementが追加されたこと（専門家選定という分岐点の理由・却下した
   代替案を意思決定系譜として記録するため。ALLOWED_STATUS_BY_ROLE["orchestrator"]={"Proposed"}
   で権限管理する）。escalate_premise_concern等の他の書き込み系ツールは引き続き含まれないこと
...
参照: docs/design/back_log/issue_backlog.md BL-148, BL-280。
"""
...
def test_call_orchestrator_tools_are_read_only_except_write_agreement():
    """[BL-280] BL-148当時は「専門家選定メタデータのみで状態を変更しない」ためwrite_agreement
    等を一切持たせない設計だったが、専門家選定自体が複数候補からの分岐点であるため、
    意思決定系譜の記録用にwrite_agreementのみ追加した（他の書き込み系ツールは対象外のまま）。"""
    src = inspect.getsource(cela_main.call_orchestrator)
    for tool_name in ("READ_PROJECT_PLAN_TOOL", "READ_DELIVERABLE_FILE_TOOL", "READ_VERIFIED_FACT_TOOL",
                       "THINK_TOOL", "WRITE_AGREEMENT_TOOL"):
        assert tool_name in src, f"call_orchestrator does not reference {tool_name}"
    for write_tool_name in (
        "ESCALATE_PREMISE_CONCERN_TOOL", "ASK_USER_QUESTION_TOOL",
