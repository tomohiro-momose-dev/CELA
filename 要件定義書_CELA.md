...
| **協調モデル** | **会話中心型（Conversational）**<br>エージェント間で長大なチャットログを流し続け、白紙から成果物を毎回書き直す。 | **成果物中心型（Artifact-Centric）**<br>中央の「共有ホワイトボード（下書き）」を囲み、差分パッチ（F-7）のみを上書き修正する。 |
| **コンテキスト管理** | **ステートフル全投入**<br>会話の「生ログ」をそのまま引き継ぐため、ログの肥大化に伴う忘却やハルシネーションを招く。 | **ステートレス能動再構成（F-8）**<br>生ログを適宜切り詰め、厳選された「5要素」のみを毎ターンSQLiteからクエリして動的アセンブルする。 |
...
| F-1.4 | **ユーザーAI（発注者）発話生成の再定義（批判的承認者）** | **【責務の完全刷新】** 進行管理や挨拶などの余計な発話を一切禁止し、**「主観物価値観（こだわり・政治・人間的都合）」の注入**と、「冷徹な批判的論理監査（論理破綻・計算ミス・都合の良い前提の看破）」に特化させる。<br><br>**【批判的読解・検閲ルール】** User AIは「承認（Approve）権限を持つ絶対的なゲートキーパー」として、Expert AIの提示したホワイトボード（下書き）に対し、以下の2軸で厳格に突っ込み（批判）を加え、容易にApproveしてはならない：<br>1. **主観的・感情的検閲**: 「このシステムフローは、スマホを持たない高齢者住民を無視した独りよがりな設計になっていないか？」<br>2. **客観的・批判的論理監査**: 「このシミュレーションの計算結果は、前提条件である年間維持費3,000万円上限と本当に矛盾していないか？ 適当な数字でごまかしていないか？」 |
| F-1.5 | ターン制御と延長 | 最大 30 ターン（初期値）の制約内で動作。QA（Reviewer）差し戻し時に 5 ターン自動延長（最大 3 回まで）。 |
